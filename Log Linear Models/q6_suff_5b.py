...
def readDevFile(devFile):
	...
	for ctr in range(0,totalSentences):
		...
		for history in enum_history:
				...
				features[(str(history[0]),history[1],history[2])] += V[('BIGRAM+TAG',sentenceList[ctr][int(history[0]) - 1],history[1],history[2])]
				...
				if (wordIndex < len(sentenceList[ctr]) - 1):
					...
					features[(str(history[0]),history[1],history[2])] += V[('TAG-WORD-1','STOP',history[2])]

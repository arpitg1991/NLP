	...
	for iterations in range(0,5):
		...
		for ctr in range(0,totalSentences):
			...
			for history in enum_history:
				...
				for i in range(2,(min(len(sentenceList[ctr][int(history[0]) - 1]),4))+1,1):
					prefix = sentenceList[ctr][int(history[0]) - 1][:i]
					features[(str(history[0]),history[1],history[2])] += featuresGlobal[('PREFIX',prefix,history[2])]
			...
			if (needToUpdate):
				...
				for i in range(0,len(sentenceList[ctr])):
					...
					for L in range(2,(min(len(sentenceList[ctr][i]),4))+1,1):
						prefix = sentenceList[ctr][i][:L]
						featuresGlobal[('PREFIX',prefix,tagSequenceFinal[i])] -= 1.0
						featuresGlobal[('PREFIX',prefix,sentenceTagList[ctr][i])] += 1.0

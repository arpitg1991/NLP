...
def readDevFile(devFile):
	...
	for ctr in range(0,totalSentences):
		...
		for history in enum_history:
			...
			#print history
			#print 'hello'
			if ('BIGRAM',history[1],history[2]) in V:
				features[(str(history[0]),history[1],history[2])] = V[('BIGRAM',history[1],history[2])]
			else:
				features[(str(history[0]),history[1],history[2])] = 0
			if ('TAG',sentenceList[ctr][int(history[0]) - 1],history[2]) in V:
				features[(str(history[0]),history[1],history[2])] = features[(str(history[0]),history[1],history[2])] + V[('TAG',sentenceList[ctr][int(history[0]) - 1],history[2])]

'''
incorporated word with prev and current tag into single feature, along with features form previous model, i.e Q6_4
'''
	...
	for iterations in range(0,5):
		...
		for ctr in range(0,totalSentences):
			...
			for history in enum_history:
				...
				'''
				This feature incorporates word,prev_tag,current_tag info into one single feature
				'''
				features[(str(history[0]),history[1],history[2])] += featuresGlobal[('BIGRAM+TAG',sentenceList[ctr][int(history[0]) - 1],history[1],history[2])]
				...
				#if ((history[1],history[2],sentenceList[ctr][wordIndex]) == ('','','') ):
			...
			if (needToUpdate):
				...
				for i in range(0,len(sentenceList[ctr])):
					...
					if ( i == 0):
						...
						featuresGlobal[('BIGRAM+TAG',sentenceList[ctr][i],'*',tagSequenceFinal[i])] -=  1.0
						featuresGlobal[('BIGRAM+TAG',sentenceList[ctr][i],'*',sentenceTagList[ctr][i])] +=  1.0
						...
						featuresGlobal[('BIGRAM+TAG',sentenceList[ctr][i],tagSequenceFinal[i-1],tagSequenceFinal[i])] -=  1.0
						featuresGlobal[('BIGRAM+TAG',sentenceList[ctr][i],sentenceTagList[ctr][i-1],sentenceTagList[ctr][i])] +=  1.0
			#else:
